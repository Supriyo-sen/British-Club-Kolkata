...
const AddMember = ({ onModal }) => {
  ...
  useEffect(() => {
    if (membershipFromDate && expiryLimit) {
      ...
      const calculatedExpiryDate = fromDate.toISOString();
      setExpiryDate(calculatedExpiryDate);

      // Check if the expiry date is valid based on the rules
      const maxAllowedDate = new Date();
      maxAllowedDate.setFullYear(maxAllowedDate.getFullYear() + 20); // Limit to 20 years in the future

      if (fromDate < now || fromDate > maxAllowedDate) {
        setExpiryDateError("Invalid expiry date selection");
      } else {
  ...
  const handlesubmit = async (e) => {
    ...
    if (idType === "Others" && idNumber === null) {
      toast.custom(
        <>
          <Toasts
            boldMessage={"Error!"}
            message={"Please input OTHERS ID Number"}
            icon={<MdError className="text-text_red" size={32} />}
          />
        </>,
        {
    ...
    try {
      const data = await addMember({
        ...
        timeStamp: membershipFromDate ? membershipFromDate : null,
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <label className="flex flex-col font-medium relative">
              ...
              {openExtend && (
                <div className="bg-primary outline-none rounded-b-lg text-text_primary absolute top-17 right-0 w-30 border-t-2 shadow-md  border-btn_primary">
